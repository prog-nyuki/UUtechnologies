(function (a) {
    a.fn.particleText = function (p) {
        ...
        function g(r, q) {
            ...
            this.r = f.clientWidth / 2.5 * 0.003;//数字が大きくなると粒子のサイズが小さくなる
        ...
        function h() {
            ...
            n = [];
            for (var t = 0; t < x; t += Math.round(x / 250)) {//粒子の数
                for (var s = 0; s < r; s += Math.round(x / 250)) {//粒子の数
                    if (u[(
                            (t + s * x) * 4
                        ) + 3] > 150) {
                        n.push(new g(t, s))
